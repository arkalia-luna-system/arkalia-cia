...
*Améliorations (23 novembre 2025) : Phase 1 (Codes couleur, calendrier, extraction enrichie), Phase 2 (Médicaments, hydratation), Phase 3 (Pathologies), Phase 4 (Améliorations IA), Sélection médecin, Refresh token, Filtre type examen, Audit log, Export/import médecins, Tests corrigés, Lint zéro erreur, NotificationService initialisé, OfflineCacheService intégré, HealthPortalAuthService OAuth complet, AdvancedSearchScreen recherche sémantique par défaut, SettingsScreen portails santé et cache, MedicationService TODO corrigé, Corrections bugs audit (13/13), Amélioration accessibilité (textes 10-12px → 14px), ErrorHelper intégré partout, Feedback visuel amélioré, Script fix_ram_overheat.sh*
