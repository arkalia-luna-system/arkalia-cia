...
class SemanticSearchService {
  ...
  double _calculateSemanticScore(Map<String, dynamic> doc, String query) {
    ...
    final docText =
        '${doc['original_name'] ?? ''} ${doc['name'] ?? ''} ${doc['category'] ?? ''} ${doc['keywords'] ?? ''}'
            .toLowerCase();
    final queryLower = query.toLowerCase();
      ...
      if (_medicalSynonyms.containsKey(entry.key)) {
        for (final synonym in _medicalSynonyms[entry.key]!) {
          if (queryLower.contains(synonym) && docText.contains(entry.key)) {
            score +=
                entry.value * 1.2; // Bonus légèrement réduit pour synonymes
          }
        }
      }
    }
    ...
    final queryWords =
        queryLower.split(' ').where((w) => w.length > 2).toList();
    int exactMatches = 0;
    ...
    for (final word in queryWords) {
      if (word.length > 4) {
        ...
        for (int i = 0; i <= docText.length - 3; i++) {
          if (docText.substring(i, min(i + word.length, docText.length)) ==
              word.substring(0, min(3, word.length))) {
            score += 0.3;
            break;
          }
        }
