...
class HealthPortalAuthService {
  ...
  static const Map<HealthPortal, String> _authUrls = {
    HealthPortal.ehealth:
        'https://iam.ehealth.fgov.be/iam-connect/oidc/authorize', // URL réelle eHealth
  ...
  static const Map<HealthPortal, String> _tokenUrls = {
    HealthPortal.ehealth:
        'https://iam.ehealth.fgov.be/iam-connect/oidc/token', // URL réelle eHealth
  ...
  Future<Map<String, dynamic>> authenticatePortal(HealthPortal portal) async {
    try {
      ...
      final clientIdKey =
          'portal_client_id_${_portalNames[portal]?.toLowerCase()}';
      ...
      final authUrl = Uri.parse(baseAuthUrl).replace(
        queryParameters: {
          'response_type': 'code',
      ...
      if (await canLaunchUrl(authUrl)) {
        ...
        return {'success': true, 'callback_url': callbackUrl, 'state': state};
      }
  ...
  Future<Map<String, dynamic>> handleOAuthCallback(
    ...
    try {
      ...
      final clientSecretKey =
          'portal_client_secret_${_portalNames[portal]?.toLowerCase()}';
          ...
          try {
            ...
            if (response.statusCode == 200) {
              ...
              if (responseData['documents'] != null) {
                data['documents'] = List<Map<String, dynamic>>.from(
                  responseData['documents'],
                );
              }
              if (responseData['consultations'] != null) {
                data['consultations'] = List<Map<String, dynamic>>.from(
                  responseData['consultations'],
                );
              }
              if (responseData['exams'] != null) {
                data['exams'] = List<Map<String, dynamic>>.from(
                  responseData['exams'],
                );
              }
              return data;
              ...
              return await _fetchGenericPortalData(
                baseUrl,
                headers,
                portal,
                portalToken,
  ...
  Future<bool> importFromPortal(
    ...
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/api/v1/health-portals/import'),
        headers: headers,
        body: jsonEncode({'portal': _portalNames[portal], 'data': data}),
      );
