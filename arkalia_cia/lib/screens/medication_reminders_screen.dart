...
class _MedicationRemindersScreenState extends State<MedicationRemindersScreen> {
  ...
  Future<bool> _isMedicationTaken(
    int medicationId,
    DateTime date,
    TimeOfDay time,
  ) async {
  ...
  Future<void> _showAddEditMedicationDialog({Medication? medication}) async {
    ...
    List<TimeOfDay> times =
        medication?.times ?? [const TimeOfDay(hour: 8, minute: 0)];
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                medication == null ? 'Médicament ajouté' : 'Médicament modifié',
              ),
            ),
          );
        }
  ...
  Future<void> _markAsTaken(Medication medication, TimeOfDay time) async {
    try {
      await _medicationService.markAsTaken(medication.id!, _selectedDate, time);
      _loadMedications();
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    final activeMedications =
        _medications.where((m) => m.isActiveOnDate(_selectedDate)).toList();
