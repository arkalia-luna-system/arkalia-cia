...
void main() {
  group('PinAuthService Tests', () {
    ...
    test('configurePin configure un PIN valide', () async {
      final result = await PinAuthService.configurePin(
        '1234',
        forceWeb: forceWeb,
      );
    ...
    test('resetPin supprime la configuration', () async {
      ...
      final verifyResult = await PinAuthService.verifyPin(
        '1234',
        forceWeb: forceWeb,
      );
    ...
    test(
      'setPinAuthEnabled active/désactive l\'authentification PIN',
      () async {
        await PinAuthService.setPinAuthEnabled(false, forceWeb: forceWeb);
        final result1 = await PinAuthService.isPinAuthEnabled(
          forceWeb: forceWeb,
        );
        expect(result1, isFalse);

        await PinAuthService.setPinAuthEnabled(true, forceWeb: forceWeb);
        final result2 = await PinAuthService.isPinAuthEnabled(
          forceWeb: forceWeb,
        );
    ...
    test('shouldAuthenticateOnStartup retourne true par défaut', () async {
      final result = await PinAuthService.shouldAuthenticateOnStartup(
        forceWeb: forceWeb,
      );
      expect(result, isTrue);
    });
    ...
    test(
      'setPinAuthOnStartup configure l\'authentification au démarrage',
      () async {
        await PinAuthService.setPinAuthOnStartup(false, forceWeb: forceWeb);
        final result1 = await PinAuthService.shouldAuthenticateOnStartup(
          forceWeb: forceWeb,
        );
        expect(result1, isFalse);

        await PinAuthService.setPinAuthOnStartup(true, forceWeb: forceWeb);
        final result2 = await PinAuthService.shouldAuthenticateOnStartup(
          forceWeb: forceWeb,
        );
        expect(result2, isTrue);
      },
