...
# S'assurer que c'est un entier (enlever les zéros en tête si nécessaire)
TIMESTAMP_CODE=$(date +%y%m%d%H%M | sed 's/^0*//')
# Si le résultat est vide (tous des zéros), utiliser 1
if [ -z "$TIMESTAMP_CODE" ]; then
    TIMESTAMP_CODE=1
fi
